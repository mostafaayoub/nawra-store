...
function AdminDash({ go }) {
  ...
  // ── Purchases (Phase 2 — Catalog/Stock refactor) ─────────────────────────
  // List uses the {rows,total,page,perPage} envelope from /api/purchases.
  // Form lives at #admin/purchases/new + /:id/edit; details at /:id (id may
  // be invoice_number PUR-XXXX or internal id — server resolves either).
  const [purchases,        setPurchases]        = useState([]);
  const [purchasesTotal,   setPurchasesTotal]   = useState(0);
  const [purchasesPage,    setPurchasesPage]    = useState(1);
  const [purchasesTab,     setPurchasesTab]     = useState("all"); // all | draft | received | cancelled
  const [purchasesFilters, setPurchasesFilters] = useState({
    q: "", supplier_id: "all", payment_status: "all", from: "", to: "", sort: "newest"
  });
  const [purchasesAggs,    setPurchasesAggs]    = useState({});
  const [purchaseDetail,   setPurchaseDetail]   = useState(null);
  // Suppliers + supplier-payments share state across pages (List → Form → Details).
  const [suppliers,        setSuppliers]        = useState([]);
  const [suppliersAggs,    setSuppliersAggs]    = useState({});
  const [supplierDetail,   setSupplierDetail]   = useState(null);
  const [supplierPayments, setSupplierPayments] = useState([]);
  const [supplierPaymentsTotal,   setSupplierPaymentsTotal]   = useState(0);
  const [supplierPaymentsAggs,    setSupplierPaymentsAggs]    = useState({});
  const [supplierPaymentsPage,    setSupplierPaymentsPage]    = useState(1);
  const [supplierPaymentsFilters, setSupplierPaymentsFilters] = useState({
  ...
  const refreshPurchases = useCallback(async () => {
    try {
      const qs = new URLSearchParams({
        status: purchasesTab,
        q: purchasesFilters.q || "",
        supplier_id: purchasesFilters.supplier_id !== "all" ? purchasesFilters.supplier_id : "",
        payment_status: purchasesFilters.payment_status !== "all" ? purchasesFilters.payment_status : "",
        from: purchasesFilters.from || "",
        to:   purchasesFilters.to || "",
        sort: purchasesFilters.sort === "newest" ? "" : purchasesFilters.sort,
        page: String(purchasesPage), perPage: "25",
      });
      [...qs.keys()].forEach(k => { if (qs.get(k) === "") qs.delete(k); });
      const r = await fetch(`/api/purchases?${qs}`);
      if (r.ok) { const d = await r.json(); setPurchases(d.rows || []); setPurchasesTotal(Number(d.total) || 0); }
    } catch {}
  }, [purchasesTab, purchasesFilters, purchasesPage]);
  const refreshPurchasesAggs = useCallback(async () => {
    try { const r = await fetch("/api/purchases/aggregates"); if (r.ok) setPurchasesAggs(await r.json()); } catch {}
  }, []);
  const refreshSuppliers = useCallback(async () => {
    try { const r = await fetch("/api/suppliers?all=1"); if (r.ok) setSuppliers(await r.json()); } catch {}
  }, []);
  const refreshSuppliersAggs = useCallback(async () => {
    try { const r = await fetch("/api/suppliers/aggregates"); if (r.ok) setSuppliersAggs(await r.json()); } catch {}
  }, []);
  const refreshSupplierDetail = useCallback(async (id) => {
    if (!id) { setSupplierDetail(null); return; }
    try { const r = await fetch(`/api/suppliers/${encodeURIComponent(id)}/details`); if (r.ok) setSupplierDetail(await r.json()); else setSupplierDetail(null); } catch { setSupplierDetail(null); }
  }, []);
  const refreshSupplierPayments = useCallback(async () => {
    try {
      const qs = new URLSearchParams({
        q: supplierPaymentsFilters.q || "",
        supplier_id: supplierPaymentsFilters.supplier_id !== "all" ? supplierPaymentsFilters.supplier_id : "",
        from: supplierPaymentsFilters.from || "",
        to:   supplierPaymentsFilters.to || "",
        page: String(supplierPaymentsPage), perPage: "25",
      });
      [...qs.keys()].forEach(k => { if (qs.get(k) === "") qs.delete(k); });
      const r = await fetch(`/api/supplier-payments?${qs}`);
      if (r.ok) { const d = await r.json(); setSupplierPayments(d.rows || []); setSupplierPaymentsTotal(Number(d.total) || 0); }
    } catch {}
  }, [supplierPaymentsFilters, supplierPaymentsPage]);
  const refreshSupplierPaymentsAggs = useCallback(async () => {
    try { const r = await fetch("/api/supplier-payments/aggregates"); if (r.ok) setSupplierPaymentsAggs(await r.json()); } catch {}
  }, []);
  const refreshPurchaseDetail = useCallback(async (key) => {
    if (!key) { setPurchaseDetail(null); return; }
    try { const r = await fetch(`/api/purchases/${encodeURIComponent(key)}`); if (r.ok) setPurchaseDetail(await r.json()); else setPurchaseDetail(null); } catch { setPurchaseDetail(null); }
  }, []);
  const refreshSupplierPaymentDetail = useCallback(async (key) => {
    if (!key) { setSupplierPaymentDetail(null); return; }
  ...
  }, [tab, purchaseFormRoute, purchaseDetailKey, refreshPurchases, refreshPurchasesAggs, refreshSuppliers]);
  useEffect(() => { refreshPurchaseDetail(purchaseDetailKey); }, [purchaseDetailKey, refreshPurchaseDetail]);
  useEffect(() => {
    // Edit mode pre-fetches the existing invoice so the form can hydrate.
    if (purchaseFormRoute && purchaseFormRoute.mode === "edit") refreshPurchaseDetail(purchaseFormRoute.id);
  }, [purchaseFormRoute && purchaseFormRoute.mode, purchaseFormRoute && purchaseFormRoute.id, refreshPurchaseDetail]); // eslint-disable-line react-hooks/exhaustive-deps
  useEffect(() => {
    if (tab === "suppliers" && !supplierDetailId) { refreshSuppliers(); refreshSuppliersAggs(); }
  }, [tab, supplierDetailId, refreshSuppliers, refreshSuppliersAggs]);
  useEffect(() => { refreshSupplierDetail(supplierDetailId); }, [supplierDetailId, refreshSupplierDetail]);
  useEffect(() => {
    if (tab === "supplier-payments" && !supplierPaymentFormRoute && !supplierPaymentDetailKey) {
      refreshSupplierPayments(); refreshSupplierPaymentsAggs(); refreshSuppliers();
    }
  }, [tab, supplierPaymentFormRoute, supplierPaymentDetailKey, refreshSupplierPayments, refreshSupplierPaymentsAggs, refreshSuppliers]);
  useEffect(() => { refreshSupplierPaymentDetail(supplierPaymentDetailKey); }, [supplierPaymentDetailKey, refreshSupplierPaymentDetail]);
    ...
    <div style={{direction:"rtl",minHeight:"100vh",background:ui.pageBg,fontFamily:ui.fontBody}}>
      <div style={{display:"grid",gridTemplateColumns:mob?"1fr":"200px 1fr",minHeight:"100vh"}}>
        ...
        <main style={{padding:mob?"14px":"20px 24px",overflow:"auto"}}>
          ...
          {/* ─── PURCHASES (Catalog/Stock refactor — Phase 2) ──────────────── */}
          {tab === "purchases" && !purchaseFormRoute && !purchaseDetailKey && (() => {
            const ag      = purchasesAggs || {};
            const counts  = ag.counts || {};
            const statusBadge = (s) => {
            ...
            const paymentBadge = (s, paid, total) => {
              if (s === "paid")    return { bg:"#DCFCE7", fg:"#15803D", l:"مدفوعة" };
              if (s === "partial") return { bg:"#FEF3C7", fg:"#92400E", l:`${(Number(paid)||0).toLocaleString()} / ${(Number(total)||0).toLocaleString()}` };
              return { bg:"#FEE2E2", fg:"#B91C1C", l:"غير مدفوعة" };
            };
            const methodLabel = (m) => ({
              cash:'كاش', transfer:'تحويل', card:'فيزا', deferred:'آجل', opening_balance:'رصيد افتتاحي'
            }[m] || m || '—');
            const purTabs = [
              ["all",       "الكل",     counts.total     || 0],
              ["draft",     "مسودة",   counts.draft     || 0],
              ["received",  "مستلمة",  counts.received  || 0],
              ["cancelled", "ملغاة",   counts.cancelled || 0],
            ];
            const exportCsv = () => {
              const head = ["رقم الفاتورة","المورد","عدد البنود","الإجمالي","تاريخ الفاتورة","تاريخ الاستلام","طريقة الدفع","حالة الدفع","الحالة"];
              const esc = (v) => { const x = v == null ? "" : String(v); return /[",\n]/.test(x) ? `"${x.replace(/"/g,'""')}"` : x; };
              const lines = [head.join(",")];
              (purchases || []).forEach(r => lines.push([
                r.invoice_number, r.supplier_name || (r.is_opening_balance?'رصيد افتتاحي':'—'),
                r.items_count, (Number(r.total)||0).toLocaleString(),
                r.invoice_date || "", r.received_date || "", methodLabel(r.payment_method),
                r.payment_status, statusBadge(r.status).l,
              ].map(esc).join(",")));
              const blob = new Blob(["﻿" + lines.join("\n")], { type:"text/csv;charset=utf-8;" });
              const url = URL.createObjectURL(blob);
              const a = document.createElement("a"); a.href = url; a.download = `purchases_${Date.now()}.csv`;
              document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
            };
            const totalPages = Math.max(1, Math.ceil((purchasesTotal || 0) / 25));
            return (
              <div>
                {/* Top bar */}
                <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:14,flexWrap:"wrap",gap:10}}>
                  <div>
                    <h2 style={{fontSize:18,fontWeight:600,color:ui.text,fontFamily:ui.fontBody,margin:0}}>المشتريات</h2>
                    <div style={{fontSize:12,color:ui.textSub,fontFamily:ui.fontBody,marginTop:2}}>إدارة فواتير شراء البضاعة</div>
                  </div>
                  <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                    <button onClick={exportCsv}
                      style={{padding:"8px 14px",background:ui.cardBg,color:ui.text,border:ui.border,borderRadius:6,fontSize:12.5,cursor:"pointer",fontFamily:ui.fontBody}}>
                ...
                {/* KPI cards (5) */}
                <div style={{display:"grid",gridTemplateColumns:mob?"1fr 1fr":"repeat(5,1fr)",gap:10,marginBottom:14}}>
                  <div style={{background:ui.cardBg,border:ui.border,borderRadius:ui.radius,padding:"14px 16px",borderTop:"3px solid #3B82F6"}}>
                    <div style={{fontSize:11.5,color:ui.textSub,fontFamily:ui.fontBody,marginBottom:5}}>إجمالي مشتريات الشهر</div>
                    <div style={{fontSize:mob?17:21,color:ui.text,fontFamily:ui.fontHead,fontWeight:500}}>
                  ...
                  <div style={{background:ui.cardBg,border:ui.border,borderRadius:ui.radius,padding:"14px 16px",borderTop:"3px solid #6366F1"}}>
                    <div style={{fontSize:11.5,color:ui.textSub,fontFamily:ui.fontBody,marginBottom:5}}>عدد الفواتير</div>
                    <div style={{fontSize:mob?17:21,color:ui.text,fontFamily:ui.fontHead,fontWeight:500}}>{ag.count_month || 0}</div>
                  </div>
                  <div style={{background:ui.cardBg,border:ui.border,borderRadius:ui.radius,padding:"14px 16px",borderTop:"3px solid #0EA5E9"}}>
                    <div style={{fontSize:11.5,color:ui.textSub,fontFamily:ui.fontBody,marginBottom:5}}>متوسط قيمة الفاتورة</div>
                    <div style={{fontSize:mob?17:21,color:ui.text,fontFamily:ui.fontHead,fontWeight:500}}>
                      {(ag.avg_invoice_month || 0).toLocaleString()} <span style={{fontSize:11,color:ui.textSub,fontFamily:ui.fontBody}}>ج</span>
                    </div>
                  </div>
                  <div style={{background:ui.cardBg,border:ui.border,borderRadius:ui.radius,padding:"14px 16px",borderTop:`3px solid ${(ag.unpaid_count||0) > 0 ? "#DC2626" : "#16A34A"}`}}>
                    <div style={{fontSize:11.5,color:ui.textSub,fontFamily:ui.fontBody,marginBottom:5}}>فواتير غير مدفوعة</div>
                    <div style={{fontSize:mob?17:21,color:ui.text,fontFamily:ui.fontHead,fontWeight:500}}>
                      {ag.unpaid_count || 0}
                      <span style={{fontSize:11,color:ui.textSub,fontFamily:ui.fontBody,marginInlineStart:6}}>·</span>
                      <span style={{fontSize:13,color:ui.textSub,fontFamily:ui.fontBody,marginInlineStart:4}}>{(ag.unpaid_total||0).toLocaleString()} ج</span>
                    </div>
                  </div>
                  <div style={{background:ui.cardBg,border:ui.border,borderRadius:ui.radius,padding:"14px 16px",borderTop:"3px solid #9333EA"}}>
                    <div style={{fontSize:11.5,color:ui.textSub,fontFamily:ui.fontBody,marginBottom:5}}>أعلى مورد (الشهر)</div>
                    <div style={{fontSize:mob?14:15,color:ui.text,fontFamily:ui.fontBody,fontWeight:500,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>
                      {ag.top_supplier ? ag.top_supplier.name : <span style={{color:ui.textSub,fontWeight:400}}>—</span>}
                    </div>
                    {ag.top_supplier && (
                      <div style={{fontSize:11.5,color:ui.textSub,fontFamily:ui.fontBody,marginTop:3}}>{(ag.top_supplier.total||0).toLocaleString()} ج</div>
                    )}
                  </div>
                </div>
                ...
                <div style={{background:ui.cardBg,border:ui.border,borderRadius:ui.radius,padding:"6px 6px",marginBottom:10,display:"flex",gap:4,overflowX:"auto"}}>
                  {purTabs.map(([k,l,n]) => (
                    <button key={k} onClick={()=>{ setPurchasesTab(k); setPurchasesPage(1); }}
                      style={{padding:"7px 14px",border:"none",cursor:"pointer",borderRadius:6,
                        background: purchasesTab===k ? ui.text : "transparent",
                        color:      purchasesTab===k ? "#fff"   : ui.textSub,
                        fontSize:12, fontFamily:ui.fontBody, whiteSpace:"nowrap"}}>
                      {l} ({n})
                    </button>
                  ))}
                ...
                {/* Filter bar */}
                <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:10}}>
                  <input value={purchasesFilters.q} onChange={e=>{setPurchasesFilters({...purchasesFilters,q:e.target.value}); setPurchasesPage(1);}}
                    placeholder="بحث برقم الفاتورة / المورد / المنتج"
                    style={{padding:"8px 11px",border:ui.border,borderRadius:6,background:ui.cardBg,fontFamily:ui.fontBody,fontSize:12.5,color:ui.text,outline:"none",direction:"rtl",minWidth:240,flex:1}}/>
                  <select value={purchasesFilters.payment_status} onChange={e=>{setPurchasesFilters({...purchasesFilters,payment_status:e.target.value}); setPurchasesPage(1);}}
                    style={{padding:"8px 11px",border:ui.border,borderRadius:6,background:ui.cardBg,fontFamily:ui.fontBody,fontSize:12.5,color:ui.text,outline:"none"}}>
                  ...
                  <select value={purchasesFilters.supplier_id} onChange={e=>{setPurchasesFilters({...purchasesFilters,supplier_id:e.target.value}); setPurchasesPage(1);}}
                    style={{padding:"8px 11px",border:ui.border,borderRadius:6,background:ui.cardBg,fontFamily:ui.fontBody,fontSize:12.5,color:ui.text,outline:"none"}}>
                    <option value="all">كل الموردين</option>
                    {(suppliers || []).map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                  </select>
                  <input type="date" value={purchasesFilters.from} onChange={e=>{setPurchasesFilters({...purchasesFilters,from:e.target.value}); setPurchasesPage(1);}}
                    title="من تاريخ"
                    style={{padding:"8px 11px",border:ui.border,borderRadius:6,background:ui.cardBg,fontFamily:ui.fontBody,fontSize:12.5,color:ui.text,outline:"none"}}/>
                  <input type="date" value={purchasesFilters.to} onChange={e=>{setPurchasesFilters({...purchasesFilters,to:e.target.value}); setPurchasesPage(1);}}
                    title="إلى تاريخ"
                    style={{padding:"8px 11px",border:ui.border,borderRadius:6,background:ui.cardBg,fontFamily:ui.fontBody,fontSize:12.5,color:ui.text,outline:"none"}}/>
                  <select value={purchasesFilters.sort} onChange={e=>setPurchasesFilters({...purchasesFilters,sort:e.target.value})}
                    style={{padding:"8px 11px",border:ui.border,borderRadius:6,background:ui.cardBg,fontFamily:ui.fontBody,fontSize:12.5,color:ui.text,outline:"none"}}>
                    <option value="newest">الأحدث</option>
                    <option value="highest">الأعلى قيمة</option>
                    <option value="due">الأقدم استحقاقاً</option>
                  </select>
                </div>

                {/* Result count */}
                <div style={{fontSize:12.5,color:ui.textSub,fontFamily:ui.fontBody,marginBottom:8}}>
                  {purchasesTotal === 0 ? "لا توجد فواتير شراء" : `${purchases.length} من ${purchasesTotal} فاتورة`}
                </div>
                ...
                {(counts.total || 0) === 0 ? (
                  <div style={{background:ui.cardBg,border:ui.border,borderRadius:ui.radius,padding:"56px 20px",textAlign:"center"}}>
                    <div style={{fontSize:42,marginBottom:10}}>📋</div>
                    <div style={{fontSize:15,color:ui.text,fontFamily:ui.fontBody,fontWeight:600,marginBottom:6}}>لا توجد فواتير شراء بعد</div>
                    <div style={{fontSize:12.5,color:ui.textSub,fontFamily:ui.fontBody,maxWidth:420,margin:"0 auto 14px"}}>
                      ابدأ بإضافة أول فاتورة شراء لتسجيل الكمية والتكلفة في المخزون
                    </div>
                    <button onClick={goPurchaseNew}
                  ...
                  <div style={{background:ui.cardBg,border:ui.border,borderRadius:ui.radius,overflow:"hidden",overflowX:"auto"}}>
                    <table style={{width:"100%",borderCollapse:"collapse",direction:"rtl",fontFamily:ui.fontBody,minWidth:1100}}>
                      <thead>
                        <tr style={{background:ui.sideBg,borderBottom:`0.5px solid #E5E5E5`}}>
                          {["رقم الفاتورة","المورد","عدد البنود","الإجمالي","تاريخ الفاتورة","تاريخ الاستلام","طريقة الدفع","حالة الدفع","الحالة","الإجراءات"].map(h => (
                            <th key={h} style={{padding:"11px 12px",textAlign:"right",fontSize:11.5,color:ui.textSub,fontWeight:500,whiteSpace:"nowrap"}}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {purchases.map(r => {
                          ...
                          const pb = paymentBadge(r.payment_status, r.amount_paid, r.total);
                          return (
                            <tr key={r.id} style={{borderTop:"0.5px solid #EEE"}}>
                              <td style={{padding:"9px 12px",fontSize:12,fontFamily:"monospace"}}>
                                <a href={`#admin/purchases/${encodeURIComponent(r.invoice_number || r.id)}`} style={{color:"#1D4ED8",textDecoration:"none"}}>{r.invoice_number || "—"}</a>
                              </td>
                              <td style={{padding:"9px 12px",fontSize:13,color:ui.text}}>
                                {r.is_opening_balance ? (
                                  <span style={{fontSize:11,padding:"2px 8px",borderRadius:20,background:"#EFF6FF",color:"#1D4ED8",fontFamily:ui.fontBody}}>رصيد افتتاحي</span>
                                ) : r.supplier_id ? (
                                  <a href={`#admin/suppliers/${encodeURIComponent(r.supplier_id)}`} style={{color:"#1D4ED8",textDecoration:"none"}}>{r.supplier_name || "—"}</a>
                                ) : "—"}
                              </td>
                              <td style={{padding:"9px 12px",fontSize:12,color:ui.textSub}}>{r.items_count || 0} {r.items_count === 1 ? "منتج" : "منتجات"}</td>
                              <td style={{padding:"9px 12px",fontSize:12.5,color:ui.text,fontWeight:500,whiteSpace:"nowrap"}}>{(Number(r.total)||0).toLocaleString()} ج</td>
                              <td style={{padding:"9px 12px",fontSize:11.5,color:ui.textSub,whiteSpace:"nowrap"}}>{r.invoice_date || "—"}</td>
                              <td style={{padding:"9px 12px",fontSize:11.5,color:ui.textSub,whiteSpace:"nowrap"}}>{(r.received_date || "").slice(0,10) || "—"}</td>
                              <td style={{padding:"9px 12px",fontSize:11.5,color:ui.textSub,whiteSpace:"nowrap"}}>{methodLabel(r.payment_method)}</td>
                              <td style={{padding:"9px 12px"}}>
                                <span style={{fontSize:10.5,padding:"3px 10px",borderRadius:20,background:pb.bg,color:pb.fg,fontFamily:ui.fontBody,whiteSpace:"nowrap"}}>{pb.l}</span>
                              </td>
                              <td style={{padding:"9px 12px"}}>
                                <span style={{fontSize:10.5,padding:"3px 10px",borderRadius:20,background:sb.bg,color:sb.fg,fontFamily:ui.fontBody,whiteSpace:"nowrap"}}>{sb.l}</span>
                              </td>
                              <td style={{padding:"9px 12px",textAlign:"left",whiteSpace:"nowrap"}}>
                                <a href={`#admin/purchases/${encodeURIComponent(r.invoice_number || r.id)}`}
                                  title="عرض التفاصيل"
                                ...
                                {r.status === "draft" && (
                                  <a href={`#admin/purchases/${encodeURIComponent(r.id)}/edit`}
                                    title="تعديل"
                                    style={{background:"transparent",border:"none",cursor:"pointer",padding:4,color:"#6B7280",fontSize:13,textDecoration:"none",marginInlineStart:4}}>
                                    ✏️
                                  </a>
                                )}
                              </td>
                ...
                {totalPages > 1 && (
                  <div style={{display:"flex",justifyContent:"center",alignItems:"center",gap:8,marginTop:12,fontFamily:ui.fontBody,fontSize:12.5}}>
                    <button disabled={purchasesPage <= 1} onClick={() => setPurchasesPage(purchasesPage - 1)}
                      style={{padding:"5px 11px",border:ui.border,background:ui.cardBg,borderRadius:5,cursor:purchasesPage <= 1 ? "not-allowed" : "pointer",opacity:purchasesPage <= 1 ? 0.5 : 1,color:ui.text}}>السابق</button>
                    <span style={{color:ui.textSub}}>{purchasesPage} / {totalPages}</span>
                    <button disabled={purchasesPage >= totalPages} onClick={() => setPurchasesPage(purchasesPage + 1)}
                      style={{padding:"5px 11px",border:ui.border,background:ui.cardBg,borderRadius:5,cursor:purchasesPage >= totalPages ? "not-allowed" : "pointer",opacity:purchasesPage >= totalPages ? 0.5 : 1,color:ui.text}}>التالي</button>
                  </div>
                )}
              </div>
            );
          ...
          {tab === "purchases" && purchaseFormRoute && (
            (purchaseFormRoute.mode === "edit" && !purchaseDetail) ? (
              ...
              <PurchaseInvoiceForm
                mode={purchaseFormRoute.mode}
                initial={purchaseFormRoute.mode === "edit" ? purchaseDetail : null}
                suppliers={suppliers}
                products={dbProducts}
                authUser={authUser}
          ...
          {tab === "suppliers" && !supplierDetailId && (() => {
            ...
            // Local filters/sorts (no pagination — supplier counts stay modest)
            const filtered = list
              .filter(s => {
                const q = (purchasesFilters.q || "").toLowerCase().trim();
                if (q) {
            ...
            return (
              <div>
                ...
                <div style={{display:"grid",gridTemplateColumns:mob?"1fr 1fr":"repeat(4,1fr)",gap:10,marginBottom:14}}>
                  <div style={{background:ui.cardBg,border:ui.border,borderRadius:ui.radius,padding:"14px 16px",borderTop:"3px solid #6366F1"}}>
                    <div style={{fontSize:11.5,color:ui.textSub,fontFamily:ui.fontBody,marginBottom:5}}>إجمالي الموردين</div>
                    <div style={{fontSize:mob?17:21,color:ui.text,fontFamily:ui.fontHead,fontWeight:500}}>{ag.total_suppliers || 0}</div>
                  </div>
                  <div style={{background:ui.cardBg,border:ui.border,borderRadius:ui.radius,padding:"14px 16px",borderTop:"3px solid #16A34A"}}>
                    <div style={{fontSize:11.5,color:ui.textSub,fontFamily:ui.fontBody,marginBottom:5}}>نشطين (آخر 90 يوم)</div>
                    <div style={{fontSize:mob?17:21,color:ui.text,fontFamily:ui.fontHead,fontWeight:500}}>{ag.active_suppliers || 0}</div>
                  </div>
                  <div style={{background:ui.cardBg,border:ui.border,borderRadius:ui.radius,padding:"14px 16px",borderTop:`3px solid ${(ag.total_payables||0) > 0 ? "#DC2626" : "#16A34A"}`}}>
                    <div style={{fontSize:11.5,color:ui.textSub,fontFamily:ui.fontBody,marginBottom:5}}>إجمالي مستحقات الموردين</div>
                    <div style={{fontSize:mob?17:21,color:ui.text,fontFamily:ui.fontHead,fontWeight:500}}>
                      {(ag.total_payables || 0).toLocaleString()} <span style={{fontSize:11,color:ui.textSub,fontFamily:ui.fontBody}}>ج</span>
                    </div>
                  </div>
                  <div style={{background:ui.cardBg,border:ui.border,borderRadius:ui.radius,padding:"14px 16px",borderTop:"3px solid #0EA5E9"}}>
                    <div style={{fontSize:11.5,color:ui.textSub,fontFamily:ui.fontBody,marginBottom:5}}>متوسط مدة السداد</div>
                    <div style={{fontSize:mob?17:21,color:ui.text,fontFamily:ui.fontHead,fontWeight:500}}>
                      {ag.avg_payment_days == null ? "—" : `${ag.avg_payment_days}`}
                      {ag.avg_payment_days != null && <span style={{fontSize:11,color:ui.textSub,marginInlineStart:4,fontFamily:ui.fontBody}}>يوم</span>}
                    </div>
                  </div>
                </div>

                {/* Filter bar (search only — kept simple per existing convention) */}
                <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:10}}>
                  <input value={purchasesFilters.q} onChange={e=>setPurchasesFilters({...purchasesFilters, q:e.target.value})}
                    placeholder="بحث باسم المورد / الهاتف / الإيميل"
                    style={{padding:"8px 11px",border:ui.border,borderRadius:6,background:ui.cardBg,fontFamily:ui.fontBody,fontSize:12.5,color:ui.text,outline:"none",direction:"rtl",minWidth:240,flex:1}}/>
                </div>

                {/* Result count */}
                <div style={{fontSize:12.5,color:ui.textSub,fontFamily:ui.fontBody,marginBottom:8}}>
                  {filtered.length === 0 ? "لا يوجد موردين" : `${filtered.length} مورد`}
                </div>
                ...
                {list.length === 0 ? (
                  ...
                  <div style={{background:ui.cardBg,border:ui.border,borderRadius:ui.radius,overflow:"hidden",overflowX:"auto"}}>
                    <table style={{width:"100%",borderCollapse:"collapse",direction:"rtl",fontFamily:ui.fontBody,minWidth:900}}>
                      <thead>
                        <tr style={{background:ui.sideBg,borderBottom:`0.5px solid #E5E5E5`}}>
                          {["اسم المورد","الهاتف","الإيميل","عدد الفواتير","إجمالي المشتريات","الرصيد","آخر تعامل","الحالة","الإجراءات"].map(h => (
                            <th key={h} style={{padding:"11px 12px",textAlign:"right",fontSize:11.5,color:ui.textSub,fontWeight:500,whiteSpace:"nowrap"}}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                        ...
                        {filtered.map(s => {
                          ...
                          return (
                            <tr key={s.id} style={{borderTop:"0.5px solid #EEE"}}>
                              <td style={{padding:"9px 12px",fontSize:13,color:ui.text}}>
                                <a href={`#admin/suppliers/${encodeURIComponent(s.id)}`} style={{color:"#1D4ED8",textDecoration:"none",fontWeight:500}}>{s.name}</a>
                              </td>
                              <td style={{padding:"9px 12px",fontSize:11.5,color:ui.textSub,fontFamily:"monospace"}}>
                                {s.phone ? (
                                  <a href={`https://wa.me/2${String(s.phone).replace(/[^\d]/g,"")}`} target="_blank" rel="noreferrer"
                                    style={{color:"#1D4ED8",textDecoration:"none"}}>{s.phone}</a>
                                ) : "—"}
                              </td>
                              <td style={{padding:"9px 12px",fontSize:11.5,color:ui.textSub}}>{s.email || "—"}</td>
                              <td style={{padding:"9px 12px",fontSize:12,color:ui.text,fontFamily:"monospace"}}>{s.invoice_count || 0}</td>
                              <td style={{padding:"9px 12px",fontSize:12.5,color:ui.text,fontWeight:500,whiteSpace:"nowrap"}}>{(Number(s.total_purchases)||0).toLocaleString()} ج</td>
                              <td style={{padding:"9px 12px",fontSize:12.5,fontWeight:500,whiteSpace:"nowrap",color: bal > 0 ? "#B91C1C" : "#15803D"}}>
                                {bal > 0 ? `لك عليه ${bal.toLocaleString()}` : bal < 0 ? `له عليك ${Math.abs(bal).toLocaleString()}` : "—"} {bal !== 0 ? "ج" : ""}
                              </td>
                              <td style={{padding:"9px 12px",fontSize:11.5,color:ui.textSub,whiteSpace:"nowrap"}}>{s.last_invoice_date || "—"}</td>
                              <td style={{padding:"9px 12px"}}>
                                <span style={{fontSize:10.5,padding:"3px 10px",borderRadius:20,background:s.active?"#DCFCE7":"#F3F4F6",color:s.active?"#15803D":"#525252",fontFamily:ui.fontBody,whiteSpace:"nowrap"}}>
          ...
          {/* Supplier details — /admin/suppliers/:id */}
          {tab === "suppliers" && supplierDetailId && (() => {
            const s = supplierDetail;
            if (!s || !s.id) {
              return <div style={{padding:30,textAlign:"center",color:ui.textSub,fontFamily:ui.fontBody,fontSize:13}}>جاري تحميل المورد...</div>;
            }
            const sum     = s.summary || {};
            const invs    = Array.isArray(s.invoices) ? s.invoices : [];
            const pays    = Array.isArray(s.payments) ? s.payments : [];
            const topProd = Array.isArray(s.top_products) ? s.top_products : [];
            const bal     = Number(sum.balance) || 0;
            const card    = { background:ui.cardBg, border:ui.border, borderRadius:ui.radius, padding:"14px 16px", marginBottom:10 };
            const cardTitle = { fontSize:12.5,fontWeight:600,color:ui.text,fontFamily:ui.fontBody,marginBottom:10,paddingBottom:6,borderBottom:"0.5px solid #EEE" };
            const kv = (k, v, color) => (
              <div style={{display:"flex",justifyContent:"space-between",padding:"5px 0",fontSize:12.5,fontFamily:ui.fontBody}}>
                <span style={{color:ui.textSub}}>{k}</span>
                <span style={{fontWeight:500,color: color || ui.text, fontFamily: typeof v === "string" && /[0-9,]/.test(v) ? "monospace" : ui.fontBody}}>{v}</span>
              </div>
            );
            return (
              <div>
                ...
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14,flexWrap:"wrap",gap:10}}>
                  <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
                    <button onClick={goSuppliers}
                      style={{background:"transparent",border:"none",cursor:"pointer",color:ui.textSub,fontSize:13,padding:"4px 8px",fontFamily:ui.fontBody}}>← العودة للقائمة</button>
                    <h2 style={{fontSize:18,fontWeight:600,color:ui.text,fontFamily:ui.fontBody,margin:0}}>{s.name}</h2>
                    <span style={{fontSize:11,padding:"3px 10px",borderRadius:20,background:s.active?"#DCFCE7":"#F3F4F6",color:s.active?"#15803D":"#525252",fontFamily:ui.fontBody}}>
                      {s.active ? "نشط" : "غير نشط"}
                    </span>
                  </div>
                  <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                    <a href="#admin/purchases/new"
                      style={{padding:"7px 13px",background:ui.text,color:"#fff",border:"none",borderRadius:5,fontSize:12,textDecoration:"none",fontFamily:ui.fontBody,fontWeight:500}}>
                      + فاتورة شراء جديدة
                    </a>
                    <button onClick={goSupplierPaymentNew}
                      style={{padding:"7px 13px",background:"#0EA5E9",color:"#fff",border:"none",borderRadius:5,fontSize:12,cursor:"pointer",fontFamily:ui.fontBody,fontWeight:500}}>
                      + تسجيل دفعة
                    </button>
                  </div>
                </div>
                ...
                <div style={{display:"grid",gridTemplateColumns:mob?"1fr":"2fr 1fr",gap:10,alignItems:"start"}}>
                  ...
                  <div>
                    ...
                    <div style={card}>
                      <div style={cardTitle}>ملخص الحساب</div>
                      {kv("إجمالي المشتريات (كل العمر)", `${(sum.lifetime_purchases || 0).toLocaleString()} ج`)}
                      {kv("إجمالي المدفوعات (كل العمر)", `${(sum.lifetime_payments || 0).toLocaleString()} ج`)}
                      {kv("الرصيد الحالي",
                        bal > 0 ? `لك عليه ${bal.toLocaleString()} ج` : bal < 0 ? `له عليك ${Math.abs(bal).toLocaleString()} ج` : "—",
                        bal > 0 ? "#B91C1C" : bal < 0 ? "#15803D" : ui.textSub)}
                      {kv("متوسط قيمة الفاتورة", `${(sum.avg_invoice_value || 0).toLocaleString()} ج`)}
                      {kv("عدد الفواتير", String(sum.invoice_count || 0))}
                      {sum.first_invoice && kv("أول تعامل", sum.first_invoice)}
                      {sum.last_invoice && kv("آخر تعامل", sum.last_invoice)}
                    </div>

                    {/* Card 2 — Invoice history */}
                    <div style={card}>
                      <div style={cardTitle}>سجل الفواتير ({invs.length})</div>
                      {invs.length === 0 ? (
                        <div style={{fontSize:12,color:ui.textSub,fontFamily:ui.fontBody,fontStyle:"italic"}}>لا توجد فواتير</div>
                      ) : (
                        <div style={{overflowX:"auto"}}>
                          <table style={{width:"100%",borderCollapse:"collapse",fontFamily:ui.fontBody,fontSize:12,minWidth:500}}>
                            <thead>
                              <tr style={{background:ui.sideBg}}>
                                {["رقم","التاريخ","الإجمالي","المدفوع","حالة الدفع","الحالة"].map(h => (
                                  <th key={h} style={{padding:"7px 8px",textAlign:"right",fontSize:11,color:ui.textSub,fontWeight:500,whiteSpace:"nowrap"}}>{h}</th>
                                ))}
                              </tr>
                            </thead>
                              ...
                              {invs.map((inv, i) => (
                                <tr key={i} style={{borderTop:"0.5px solid #EEE"}}>
                                  <td style={{padding:"7px 8px",fontFamily:"monospace"}}>
                                    <a href={`#admin/purchases/${encodeURIComponent(inv.invoice_number || inv.id)}`} style={{color:"#1D4ED8",textDecoration:"none"}}>{inv.invoice_number || inv.id}</a>
                                  </td>
                                  <td style={{padding:"7px 8px",color:ui.textSub}}>{inv.invoice_date || "—"}</td>
                                  <td style={{padding:"7px 8px",fontFamily:"monospace"}}>{(Number(inv.total)||0).toLocaleString()} ج</td>
                                  <td style={{padding:"7px 8px",fontFamily:"monospace"}}>{(Number(inv.amount_paid)||0).toLocaleString()} ج</td>
                                  <td style={{padding:"7px 8px",fontSize:10.5}}>
                                    <span style={{padding:"2px 8px",borderRadius:20,background:inv.payment_status==='paid'?'#DCFCE7':inv.payment_status==='partial'?'#FEF3C7':'#FEE2E2',color:inv.payment_status==='paid'?'#15803D':inv.payment_status==='partial'?'#92400E':'#B91C1C'}}>
                                      {inv.payment_status==='paid'?'مدفوعة':inv.payment_status==='partial'?'جزئية':'غير مدفوعة'}
                                    </span>
                                  </td>
                                  <td style={{padding:"7px 8px",fontSize:10.5}}>
                                    <span style={{padding:"2px 8px",borderRadius:20,background:inv.status==='cancelled'?'#FEE2E2':inv.status==='received'?'#DCFCE7':'#F3F4F6',color:inv.status==='cancelled'?'#B91C1C':inv.status==='received'?'#15803D':'#525252'}}>
                                      {inv.status==='cancelled'?'ملغاة':inv.status==='received'?'مستلمة':'مسودة'}
                                    </span>
                                  </td>
                                </tr>
                              ))}
                    ...
                    <div style={card}>
                      <div style={cardTitle}>سجل الدفعات ({pays.length})</div>
                      {pays.length === 0 ? (
                        <div style={{fontSize:12,color:ui.textSub,fontFamily:ui.fontBody,fontStyle:"italic"}}>لم تسجل دفعات بعد</div>
                      ) : (
                        <div style={{display:"flex",flexDirection:"column",gap:6}}>
                          {pays.map((p, i) => (
                            <a key={i} href={`#admin/supplier-payments/${encodeURIComponent(p.payment_number || p.id)}`}
                              style={{display:"flex",justifyContent:"space-between",padding:"7px 10px",background:"#F9FAFB",borderRadius:5,fontSize:12,color:ui.text,fontFamily:ui.fontBody,textDecoration:"none"}}>
                              <span>
                                <span style={{fontFamily:"monospace",color:"#1D4ED8"}}>{p.payment_number || "—"}</span>
                                <span style={{color:ui.textSub,marginInlineStart:8}}>{p.payment_date || ""}</span>
                                {p.payment_method && <span style={{color:ui.textSub,marginInlineStart:8}}>· {p.payment_method}</span>}
                              </span>
                              <span style={{fontFamily:"monospace",fontWeight:500}}>{(Number(p.amount)||0).toLocaleString()} ج</span>
                            </a>
                          ))}
                        </div>
                      )}
                    ...
                    {/* Card 4 — Top products supplied */}
                    {topProd.length > 0 && (
                      <div style={card}>
                        <div style={cardTitle}>المنتجات المُورّدة ({topProd.length})</div>
                        <div style={{overflowX:"auto"}}>
                          <table style={{width:"100%",borderCollapse:"collapse",fontFamily:ui.fontBody,fontSize:12,minWidth:500}}>
                            <thead>
                              <tr style={{background:ui.sideBg}}>
                                {["المنتج","الكمية الإجمالية","متوسط السعر","آخر سعر","آخر توريد"].map(h => (
                                  <th key={h} style={{padding:"7px 8px",textAlign:"right",fontSize:11,color:ui.textSub,fontWeight:500,whiteSpace:"nowrap"}}>{h}</th>
                                ))}
                              </tr>
                            </thead>
                            <tbody>
                              {topProd.map((p, i) => (
                                <tr key={i} style={{borderTop:"0.5px solid #EEE"}}>
                                  <td style={{padding:"7px 8px"}}>{p.name}</td>
                                  <td style={{padding:"7px 8px",fontFamily:"monospace"}}>{p.qty_total}</td>
                                  <td style={{padding:"7px 8px",fontFamily:"monospace"}}>{(p.avg_cost || 0).toLocaleString()} ج</td>
                                  <td style={{padding:"7px 8px",fontFamily:"monospace"}}>{(p.last_cost || 0).toLocaleString()} ج</td>
                                  <td style={{padding:"7px 8px",fontSize:11,color:ui.textSub}}>{(p.last_supplied || "").slice(0,10) || "—"}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                  ...
                  <div>
                    <div style={card}>
                      <div style={cardTitle}>معلومات الاتصال</div>
                      {s.phone && (
                        <div style={{fontSize:13,fontFamily:"monospace",color:ui.text,marginBottom:4}}>
                          📱 <a href={`https://wa.me/2${String(s.phone).replace(/[^\d]/g,"")}`} target="_blank" rel="noreferrer" style={{color:"#1D4ED8",textDecoration:"none"}}>{s.phone}</a>
                        </div>
                      )}
                      {s.email && (
                        <div style={{fontSize:12.5,color:ui.text,marginBottom:4}}>
                          ✉️ <a href={`mailto:${s.email}`} style={{color:"#1D4ED8",textDecoration:"none"}}>{s.email}</a>
                        </div>
                      )}
                      {!s.phone && !s.email && (
                        <div style={{fontSize:12,color:ui.textSub,fontFamily:ui.fontBody,fontStyle:"italic"}}>لم يتم تسجيل معلومات اتصال</div>
                      )}
                    </div>
                    <div style={card}>
                      <div style={cardTitle}>التصنيف وشروط الدفع</div>
                      {kv("نوع المورد", ({
                        distributor:"موزع رسمي", merchant:"تاجر", importer:"مستورد", other:"آخر"
                      }[s.supplier_type] || s.supplier_type || "—"))}
                      {kv("شروط الدفع الافتراضية", (Number(s.default_payment_terms_days) || 0) > 0
                        ? `${s.default_payment_terms_days} يوم آجل`
                        : "كاش فوري")}
                    </div>
                  </div>
                </div>
...
// trigger AdminDash re-renders and lose focus. Used by /admin/purchases/new
// and /admin/purchases/:id/edit. Caller passes the existing invoice (for
// edit mode) and the suppliers + products lists; the form manages its own
// state and POSTs/PATCHes itself, then calls onSaved to navigate.
//
...
function PurchaseInvoiceForm({ mode, initial, suppliers, products, authUser, onSaved, onCancel, ui, mob }) {
  ...
  const [supplierId,      setSupplierId]      = useState(isEdit ? (initial.supplier_id || "")  : "");
  const [supplierInvRef,  setSupplierInvRef]  = useState(isEdit ? (initial.supplier_invoice_ref || "") : "");
  const [invoiceDate,     setInvoiceDate]     = useState(isEdit ? (initial.invoice_date  || today) : today);
  const [receivedDate,    setReceivedDate]    = useState(isEdit ? (initial.received_date?.slice(0,10) || today) : today);
  const [dueDate,         setDueDate]         = useState(isEdit ? (initial.due_date      || "") : "");
  const [shippingCost,    setShippingCost]    = useState(isEdit ? (Number(initial.shipping_cost) || 0) : 0);
  const [customsFees,     setCustomsFees]     = useState(isEdit ? (Number(initial.customs_fees)  || 0) : 0);
  const [otherCosts,      setOtherCosts]      = useState(isEdit ? (Number(initial.other_costs)   || 0) : 0);
  const [landedBasis,     setLandedBasis]     = useState(isEdit ? (initial.landed_basis  || "value") : "value");
  const [paymentMethod,   setPaymentMethod]   = useState(isEdit ? (initial.payment_method || "cash") : "cash");
  const [statusVal,       setStatusVal]       = useState(isEdit ? (initial.status || "draft") : "draft");
  const [notes,           setNotes]           = useState(isEdit ? (initial.notes  || "") : "");
  const [internalNotes,   setInternalNotes]   = useState(isEdit ? (initial.internal_notes || "") : "");
  const [items, setItems] = useState(() => {
    if (isEdit && Array.isArray(initial.items)) {
    ...
    return [{ product_id: "", quantity: 0, unit_cost: 0, notes: "" }];
  });
  const [saveBusy, setSaveBusy] = useState(false);
  const [err,      setErr]      = useState("");

  // Build a fast product lookup so each line can show current stock + WAC
  // inline as the admin types. Pulls weight_kg too for the weight-basis
  ...
  const landed   = (Number(shippingCost) || 0) + (Number(customsFees) || 0) + (Number(otherCosts) || 0);
  const total    = subtotal + landed;

  // Allocated landed cost per line — runs the same formula as the server.
  const itemsEnriched = items.map(it => {
    const lineTotal = (Number(it.quantity) || 0) * (Number(it.unit_cost) || 0);
    let allocated = 0;
    if (landed > 0 && items.length) {
      if (landedBasis === "weight") {
        const prod = prodMap.get(it.product_id);
        const w    = (Number(prod && prod.weight_kg) || 0.3) * (Number(it.quantity) || 0);
        const totalWeight = items.reduce((s, x) => {
          const p = prodMap.get(x.product_id);
          return s + ((Number(p && p.weight_kg) || 0.3) * (Number(x.quantity) || 0));
        }, 0);
        allocated = totalWeight > 0 ? (landed * (w / totalWeight)) : 0;
      } else {
        allocated = subtotal > 0 ? (landed * (lineTotal / subtotal)) : 0;
      }
    }
    const qty = Number(it.quantity) || 0;
    const effectiveUnit = qty > 0 ? (it.unit_cost + (allocated / qty)) : 0;
    return { ...it, lineTotal, allocated, effectiveUnit };
  });

  // Inventory + financial impact previews.
  const inventoryImpact = itemsEnriched
    .filter(it => it.product_id && it.quantity > 0)
    .map(it => {
      const p = prodMap.get(it.product_id) || {};
      const curStock = Number(p.stock) || 0;
      const curWac   = Number(p.weighted_average_cost) || 0;
      const newStock = curStock + (Number(it.quantity) || 0);
      // WAC = (curStock*curWac + qty*effective) / newStock
      const newWac   = newStock > 0
        ? Math.round(((curStock * curWac + (Number(it.quantity) || 0) * it.effectiveUnit) / newStock) * 100) / 100
        : 0;
      return {
        name:     p.name || "—",
        addQty:   Number(it.quantity) || 0,
        curStock, newStock,
        curWac,   newWac,
        effectiveUnit: Math.round(it.effectiveUnit * 100) / 100,
      };
    });
  ...
  // ── Item helpers ────────────────────────────────────────────────────────
  const addItem    = () => setItems(prev => [...prev, { product_id:"", quantity:0, unit_cost:0, notes:"" }]);
  const removeItem = (i) => setItems(prev => prev.filter((_, j) => j !== i));
  const patchItem  = (i, patch) => setItems(prev => prev.map((it, j) => j === i ? { ...it, ...patch } : it));

  // ── Validation ──────────────────────────────────────────────────────────
  const validate = () => {
    if (!supplierId)             return "اختر مورداً قبل الحفظ";
    if (!invoiceDate)            return "تاريخ الفاتورة مطلوب";
    if (paymentMethod === "deferred" && !dueDate)
                                 return "تاريخ الاستحقاق مطلوب للدفع الآجل";
    const validItems = items.filter(it => it.product_id);
    if (validItems.length === 0) return "أضف على الأقل بنداً واحداً (منتج + كمية + سعر)";
    for (const it of validItems) {
      if (!(Number(it.quantity)  > 0))   return "الكمية يجب أن تكون أكبر من 0";
      if (!(Number(it.unit_cost) >= 0))  return "سعر الوحدة لا يمكن أن يكون سالباً";
    }
    if (invoiceDate && receivedDate && receivedDate < invoiceDate)
                                 return "تاريخ الاستلام يجب أن يكون بعد تاريخ الفاتورة أو نفس اليوم";
    return null;
  };
  ...
  const onSave = async () => {
    ...
    try {
      const body = {
        supplier_id: supplierId,
        supplier_invoice_ref: supplierInvRef || null,
        invoice_date: invoiceDate, received_date: receivedDate,
        due_date: paymentMethod === "deferred" ? (dueDate || null) : null,
        shipping_cost: Number(shippingCost) || 0,
        customs_fees:  Number(customsFees)  || 0,
        other_costs:   Number(otherCosts)   || 0,
        landed_basis:  landedBasis,
        payment_method: paymentMethod,
        status: statusVal,
        notes: notes || null,
      ...
      const url    = isEdit ? `/api/purchases/${encodeURIComponent(initial.id)}` : "/api/purchases";
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14,flexWrap:"wrap",gap:10}}>
        <div>
          <h2 style={{fontSize:18,fontWeight:600,color:ui.text,fontFamily:ui.fontBody,margin:0}}>
            {isEdit ? `تعديل فاتورة شراء ${initial.invoice_number || ""}` : "فاتورة شراء جديدة"}
          </h2>
          <div style={{fontSize:12,color:ui.textSub,fontFamily:ui.fontBody,marginTop:2}}>
            {isEdit ? "تعديل البنود والتكاليف — اضغط حفظ لتطبيق التغييرات" : "أضف البنود والتكاليف لتسجيل الكمية والتكلفة في المخزون"}
          </div>
        </div>
        <div style={{display:"flex",gap:8}}>
          <button onClick={onCancel}
          ...
          <button onClick={onSave} disabled={saveBusy}
            style={{padding:"8px 16px",background:saveBusy?"#9CA3AF":ui.text,color:"#fff",border:"none",borderRadius:6,fontSize:12.5,cursor:saveBusy?"wait":"pointer",fontFamily:ui.fontBody,fontWeight:500}}>
            {saveBusy ? "جاري الحفظ..." : (isEdit ? "حفظ التعديلات" : "حفظ الفاتورة")}
          </button>
        </div>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:mob?"1fr":"2fr 1fr",gap:10,alignItems:"start"}}>
        ...
        <div>
          ...
          <div style={card}>
            ...
            <div style={{display:"grid",gridTemplateColumns:mob?"1fr":"1fr 1fr",gap:10}}>
              <div>
                ...
                <input value={isEdit ? (initial.invoice_number || "") : "(تلقائي عند الحفظ)"} disabled
              ...
              <div>
                <label style={lbl}>تاريخ الفاتورة <span style={{color:"#DC2626"}}>*</span></label>
              ...
              <div style={{gridColumn: mob ? "auto" : "1 / span 2"}}>
                <label style={lbl}>رقم فاتورة المورد (اختياري)</label>
                <input value={supplierInvRef} onChange={e=>setSupplierInvRef(e.target.value)}
                  placeholder="مرجع الفاتورة من المورد"
                  style={inputStyle}/>
              </div>
            </div>
          </div>
          ...
          <div style={card}>
            ...
            <div style={{overflowX:"auto"}}>
              <table style={{width:"100%",borderCollapse:"collapse",fontFamily:ui.fontBody,fontSize:12,minWidth:600}}>
                <thead>
                  <tr style={{background:ui.sideBg}}>
                    <th style={{padding:"7px 8px",textAlign:"right",fontSize:11,color:ui.textSub,fontWeight:500}}>المنتج</th>
                    <th style={{padding:"7px 8px",textAlign:"right",fontSize:11,color:ui.textSub,fontWeight:500,width:90}}>الكمية</th>
                    <th style={{padding:"7px 8px",textAlign:"right",fontSize:11,color:ui.textSub,fontWeight:500,width:110}}>سعر الوحدة</th>
                    <th style={{padding:"7px 8px",textAlign:"right",fontSize:11,color:ui.textSub,fontWeight:500,width:110}}>المجموع</th>
                    <th style={{padding:"7px 8px",width:34}}></th>
                  </tr>
                </thead>
                <tbody>
                  {items.map((it, i) => {
                    const lineT = (Number(it.quantity) || 0) * (Number(it.unit_cost) || 0);
                    const prod  = prodMap.get(it.product_id);
                    return (
                      <tr key={i} style={{borderTop:"0.5px solid #EEE"}}>
                        <td style={{padding:"6px 8px"}}>
                          <select value={it.product_id} onChange={e=>patchItem(i, { product_id:e.target.value })} style={inputStyle}>
                            <option value="">— اختر منتجاً —</option>
                            {(products || []).map(p => (
                              <option key={p.id} value={p.id}>{p.name} {p.stock != null ? `(الكمية ${p.stock})` : ""}</option>
                            ))}
                          </select>
                          {prod && (
                            <div style={{fontSize:10.5,color:ui.textSub,marginTop:3,fontFamily:"monospace"}}>
                              متوسط التكلفة الحالي: {(Number(prod.weighted_average_cost)||0).toLocaleString()} ج
                            </div>
                          )}
                        </td>
                        <td style={{padding:"6px 8px"}}>
                        ...
                        <td style={{padding:"6px 8px"}}>
                          <input type="number" value={it.unit_cost} min={0} step="0.01"
                            onChange={e=>patchItem(i, { unit_cost:e.target.value })}
                            style={numInput}/>
                        </td>
                        <td style={{padding:"6px 8px",fontFamily:"monospace",color:ui.text,fontSize:12}}>{lineT.toLocaleString()} ج</td>
                        <td style={{padding:"6px 8px",textAlign:"left"}}>
                          {items.length > 1 && (
                            <button onClick={()=>removeItem(i)} title="حذف الصف"
                              style={{background:"transparent",border:"none",color:"#DC2626",cursor:"pointer",fontSize:14,padding:2}}>🗑</button>
                          )}
                        </td>
                      </tr>
                    );
            ...
            <button onClick={addItem}
              style={{marginTop:8,padding:"7px 14px",background:"transparent",color:"#1D4ED8",border:"0.5px dashed #93C5FD",borderRadius:5,fontSize:12,cursor:"pointer",fontFamily:ui.fontBody}}>
              + إضافة بند
            </button>
          </div>
          ...
          <div style={card}>
            ...
            <div style={{display:"grid",gridTemplateColumns:mob?"1fr":"1fr 1fr 1fr",gap:10,marginBottom:10}}>
              <div>
                <label style={lbl}>تكلفة الشحن من المورد</label>
                <input type="number" min={0} step="0.01" value={shippingCost} onChange={e=>setShippingCost(e.target.value)} style={numInput}/>
              </div>
              <div>
                <label style={lbl}>الجمارك والرسوم</label>
                <input type="number" min={0} step="0.01" value={customsFees} onChange={e=>setCustomsFees(e.target.value)} style={numInput}/>
              </div>
              <div>
                <label style={lbl}>تكاليف أخرى</label>
                <input type="number" min={0} step="0.01" value={otherCosts} onChange={e=>setOtherCosts(e.target.value)} style={numInput}/>
              </div>
            </div>
            <div>
              <label style={lbl}>توزيع التكاليف على البنود</label>
              <div style={{display:"flex",gap:14,marginTop:4}}>
                <label style={{display:"flex",alignItems:"center",gap:6,fontSize:12.5,color:ui.text,fontFamily:ui.fontBody,cursor:"pointer"}}>
                  <input type="radio" checked={landedBasis === "value"} onChange={()=>setLandedBasis("value")}/>
                  نسبة من القيمة
                </label>
                <label style={{display:"flex",alignItems:"center",gap:6,fontSize:12.5,color:ui.text,fontFamily:ui.fontBody,cursor:"pointer"}}>
                  <input type="radio" checked={landedBasis === "weight"} onChange={()=>setLandedBasis("weight")}/>
                  نسبة من الوزن
                </label>
              </div>
            </div>
          ...
          <div style={card}>
            <div style={cardTitle}>الإجمالي</div>
            <div style={{display:"flex",flexDirection:"column",gap:5,fontSize:13,fontFamily:ui.fontBody,color:ui.text}}>
              <div style={{display:"flex",justifyContent:"space-between"}}>
                <span>مجموع البنود</span>
                <span style={{fontFamily:"monospace"}}>{subtotal.toLocaleString()} ج</span>
              </div>
              <div style={{display:"flex",justifyContent:"space-between",color:ui.textSub}}>
                <span>+ التكاليف الإضافية</span>
                <span style={{fontFamily:"monospace"}}>{landed.toLocaleString()} ج</span>
              </div>
              <div style={{display:"flex",justifyContent:"space-between",paddingTop:6,marginTop:4,borderTop:"1px dashed #E5E5E5",fontWeight:600,fontSize:15}}>
                <span>الإجمالي النهائي</span>
                <span style={{fontFamily:"monospace"}}>{total.toLocaleString()} ج</span>
              </div>
            </div>
          </div>

          {/* Card 5 — Attachments (placeholder until multer upload is wired) */}
          <div style={card}>
            <div style={cardTitle}>الإيصال والمرفقات</div>
            <div style={{padding:"14px 12px",background:"#F9FAFB",border:"0.5px dashed #D4D4D4",borderRadius:6,fontSize:12,color:ui.textSub,fontFamily:ui.fontBody,textAlign:"center"}}>
              📎 رفع الملفات (jpg/png/pdf) — يدعمها الخادم في مرحلة قادمة
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div style={card}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {[
                ...
                <label key={k} style={{display:"flex",alignItems:"center",gap:7,fontSize:12.5,color:ui.text,fontFamily:ui.fontBody,cursor:"pointer"}}>
                  <input type="radio" checked={paymentMethod === k} onChange={()=>setPaymentMethod(k)}/>
                  {l}
                </label>
              ))}
              {paymentMethod === "deferred" && (
                <div style={{marginTop:6,padding:"8px 10px",background:"#FFFBEB",border:"0.5px solid #FDE68A",borderRadius:5}}>
                  <label style={lbl}>تاريخ الاستحقاق <span style={{color:"#DC2626"}}>*</span></label>
                  <input type="date" value={dueDate} onChange={e=>setDueDate(e.target.value)} style={inputStyle}/>
                </div>
              )}
            </div>
            <div style={{marginTop:14,paddingTop:10,borderTop:"0.5px solid #EEE"}}>
              <label style={lbl}>حالة الفاتورة</label>
              <select value={statusVal} onChange={e=>setStatusVal(e.target.value)} style={inputStyle}>
                <option value="draft">مسودة (لا تؤثر على المخزون)</option>
                <option value="received">مستلمة (تطبق على المخزون)</option>
              </select>
            </div>
          ...
          {/* Sidebar 2 — Inventory impact preview */}
          <div style={card}>
            <div style={cardTitle}>التأثير على المخزون</div>
            {inventoryImpact.length === 0 ? (
              <div style={{fontSize:12,color:ui.textSub,fontFamily:ui.fontBody,fontStyle:"italic"}}>أضف بنوداً لرؤية التأثير</div>
            ) : (
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {inventoryImpact.map((row, i) => (
                  <div key={i} style={{fontSize:11.5,color:ui.text,fontFamily:ui.fontBody,padding:"7px 9px",background:"#F9FAFB",borderRadius:5}}>
                    <div style={{fontWeight:500,marginBottom:3}}>{row.name}</div>
                    <div style={{color:ui.textSub}}>+ {row.addQty} وحدة · المخزون بعد: <b style={{color:ui.text,fontFamily:"monospace"}}>{row.newStock}</b></div>
                    <div style={{color:ui.textSub,fontFamily:"monospace",fontSize:11}}>
                      WAC الحالي: {row.curWac.toLocaleString()} → الجديد: <b style={{color:ui.text}}>{row.newWac.toLocaleString()}</b> ج
                    </div>
                  </div>
                ))}
              </div>
          ...
          {/* Sidebar 3 — Financial impact preview */}
          <div style={card}>
            <div style={cardTitle}>التأثير المالي</div>
            <div style={{display:"flex",flexDirection:"column",gap:6,fontSize:12,color:ui.text,fontFamily:ui.fontBody}}>
              {paymentMethod === "deferred" ? (
                <div style={{padding:"7px 9px",background:"#FFFBEB",border:"0.5px solid #FDE68A",borderRadius:5}}>
                  <div style={{color:ui.textSub,fontSize:11.5,marginBottom:2}}>مستحقات للمورد ستزيد بـ</div>
                  <div style={{fontFamily:"monospace",fontWeight:600}}>{total.toLocaleString()} ج</div>
                </div>
              ) : (
                <div style={{padding:"7px 9px",background:"#FEE2E2",border:"0.5px solid #FCA5A5",borderRadius:5}}>
                  <div style={{color:ui.textSub,fontSize:11.5,marginBottom:2}}>النقد سيقل بـ</div>
                  <div style={{fontFamily:"monospace",fontWeight:600,color:"#991B1B"}}>{total.toLocaleString()} ج</div>
                </div>
              )}
              <div style={{padding:"7px 9px",background:"#DCFCE7",border:"0.5px solid #86EFAC",borderRadius:5}}>
                <div style={{color:ui.textSub,fontSize:11.5,marginBottom:2}}>أصل المخزون سيزيد بـ</div>
                <div style={{fontFamily:"monospace",fontWeight:600,color:"#15803D"}}>{total.toLocaleString()} ج</div>
              </div>
              {statusVal === "draft" && (
                <div style={{padding:"7px 9px",background:"#F3F4F6",border:"0.5px solid #D4D4D4",borderRadius:5,fontSize:11.5,color:ui.textSub,fontStyle:"italic"}}>
                  ⚠ الفاتورة مسودة — لن تطبق على المخزون أو المالية حتى الحفظ كـ"مستلمة"
                </div>
              )}
            </div>
          </div>
